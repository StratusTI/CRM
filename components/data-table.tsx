...
import { RecordPanel } from "@/components/tables/record-panel";
...
import {
  ...
  reorderResource,
  updateResource,
} from "@/src/hooks/use-resource-list";
...
function DragHandle({ handle }: { handle: SortableHandle }) {
  return (
    ...
    >
      <HugeiconsIcon icon={Drag01Icon} strokeWidth={2} className="size-4" />
...
export function DataTable<TData extends WithId>({
  ...
  slug: string;
  resource: string;
  /** Nome singular para o botão "New X". */
  createTitle: string;
  lookups: Lookups;
  ...
  const calcStorageKey = `nexo:table-calc:${slug}:${resource}`;
  ...
  const patch = React.useCallback(
    (id: string, fields: Record<string, unknown>) => {
      ...
      void updateResource(slug, resource, id, fields).then((res) => {
        if (!res.ok) {
  ...
  const removeRow = React.useCallback(
    (id: string) => {
      ...
      void deleteResource(slug, resource, id).then((res) => {
        if (!res.ok) {
  ...
  const addRow = React.useCallback(() => {
    ...
    void createResource<TData>(slug, resource, payload).then((res) => {
      if (!res.ok || !res.data) {
        toast.error(res.message ?? "Não foi possível criar.");
        return;
      }
  ...
  const tableColumns = React.useMemo<ColumnDef<TData>[]>(() => {
    ...
    return [
      drag,
      select,
      ...buildGridColumns<TData>(columns, lookups),
      actions,
    ];
  }, [columns, lookups]);
