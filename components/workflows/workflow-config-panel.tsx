...
import {
  type WorkflowNode,
  WORKFLOW_DELAY_UNITS,
  WORKFLOW_ENTITIES,
  WORKFLOW_FILTER_OPERATORS,
  WORKFLOW_FORM_FIELD_TYPES,
...
export function WorkflowConfigPanel({
  ...
  onUpdateTrigger: (data: WorkflowTriggerData | null) => void;
  ...
  if (selectedId === "trigger") {
    return (
      <PanelShell
        ...
        onClose={onClose}
        onDelete={
          trigger.data ? () => onUpdateTrigger(null) : undefined
        }
...
function TriggerForm({
  ...
  data: WorkflowTriggerData | null;
  onChange: (next: WorkflowTriggerData | null) => void;
}) {
  if (!data) {
    return (
  ...
  return (
    <div className="space-y-4">
      ...
      {data.type === "launch-manually" && (
        <p className="text-sm text-muted-foreground">
          Sem configuração — disparado pelo botão “Test” ou pelo trigger
          manual.
        </p>
      )}
      ...
      {data.type === "webhook" && (
        <Field
          ...
          hint="URL: POST /api/workflows/webhook/<token>"
        >
          <Input
            value={data.token}
    ...
    <div className="space-y-4">
      <Field label="Rótulo">
        <Input
          value={data.label ?? ""}
          onChange={(e) =>
            onChange({ ...data, label: e.target.value } as WorkflowNodeData)
          }
        />
      </Field>
      ...
      return (
        <div className="grid grid-cols-2 gap-2">
          ...
          <Field label="Unidade">
            <NativeSelect
              value={data.unit}
              options={WORKFLOW_DELAY_UNITS.map((u) => ({
                value: u,
                label: u,
              }))}
              onChange={(unit) =>
                onChange({
                  ...data,
                  unit: unit as (typeof WORKFLOW_DELAY_UNITS)[number],
                })
              }
            />
          </Field>
...
function EntitySelect({
  ...
  value: string;
  onChange: (v: (typeof WORKFLOW_ENTITIES)[number]) => void;
}) {
  return (
    <NativeSelect
      value={value}
      options={WORKFLOW_ENTITIES.map((e) => ({ value: e, label: e }))}
      onChange={(v) =>
        onChange(v as (typeof WORKFLOW_ENTITIES)[number])
      }
    />
  );
}
  ...
  return (
    <Field label="Entidade">
      <EntitySelect
        value={data.entity}
        onChange={(entity) =>
          onChange({ ...data, entity } as WorkflowNodeData)
        }
      />
    </Field>
  );
...
type ConditionRow = {
  field: string;
  operator: (typeof WORKFLOW_FILTER_OPERATORS)[number];
  value: string;
};
        ...
        {value.map((row, i) => (
          <div key={`cond-${i}`} className="grid grid-cols-12 gap-1.5">
            ...
            <div className="col-span-3">
              <NativeSelect
                value={row.operator}
                options={WORKFLOW_FILTER_OPERATORS.map((o) => ({
                  value: o,
                  label: o,
                }))}
                onChange={(v) =>
                  update(i, {
                    operator: v as (typeof WORKFLOW_FILTER_OPERATORS)[number],
                  })
                }
              />
            </div>
...
function FormFieldsEditor({
  ...
  data: Extract<WorkflowNodeData, { type: "form" }>;
  onChange: (next: WorkflowNodeData) => void;
}) {
  const update = (
    i: number,
    patch: Partial<(typeof data.fields)[number]>,
  ) => {
    ...
    <>
      ...
      <div className="space-y-2">
        ...
        {data.fields.map((f, i) => (
          <div
            key={`f-${i}`}
            className="rounded-md border bg-card/50 p-2 space-y-1.5"
          >
            <div className="flex items-center gap-1.5">
              ...
              <div className="w-28">
                <NativeSelect
                  value={f.type}
                  options={WORKFLOW_FORM_FIELD_TYPES.map((t) => ({
                    value: t,
                    label: t,
                  }))}
                  onChange={(v) =>
                    update(i, {
                      type: v as (typeof WORKFLOW_FORM_FIELD_TYPES)[number],
                    })
                  }
                />
              </div>
