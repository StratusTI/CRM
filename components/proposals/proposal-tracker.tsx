 ...
 * `/api/p/<token>/view` via `sendBeacon`. Mede tempo ativo (só com a aba
 * visível), profundidade de scroll e se o visitante chegou ao fim (o próprio
 * elemento serve de sentinela do fim do documento).
 */
export function ProposalTracker({ token }: { token: string }) {
  ...
  React.useEffect(() => {
    ...
    const url = apiUrl(`/api/p/${token}/view`);
