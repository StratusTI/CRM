...
function printAsPdf(html: string, title: string, size: PaperSize) {
  ...
  if (!win) {
    toast.error("Popup bloqueado. Permita popups para exportar em PDF.");
    return;
  }
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* Content */}
          <div className="flex flex-col gap-1.5">
            <label className="font-medium text-sm">Conteúdo</label>
            <div className="flex gap-2">
...
export function ProposalOptionsMenu({
  ...
  const makeCopy = async () => {
    try {
      const res = await fetch(apiUrl(`/api/workspaces/${slug}/proposals`), {
        method: "POST",
      ...
      toast.error(e instanceof Error ? e.message : "Erro ao criar cópia.");
    }
  };
