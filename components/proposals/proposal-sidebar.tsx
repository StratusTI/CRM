...
import type { DocStats } from "@/components/rich-text/analyze";
...
export function ProposalSidebar({
  ...
  shareUrl: string;
  metrics: ProposalMetricsDTO | null;
  onOpenMetrics: () => void;
}) {
  ...
  return (
    <aside className="flex w-72 shrink-0 flex-col gap-4 overflow-y-auto border-l bg-background/40 p-4">
      ...
      <section>
        ...
        />
        <StatRow icon={Note01Icon} label="Seções" value={String(stats.headings)} />
        <StatRow
          icon={Image01Icon}
          label="Imagens / mídia"
      ...
      <section>
        ...
        <StatRow
          ...
          label="Conclusão"
          value={
            metrics ? `${Math.round(metrics.completionRate * 100)}%` : "—"
          }
        />
        <StatRow
          icon={Timer02Icon}
