...
import { getProposalMetrics } from "@/src/hooks/use-proposal";
import type { ProposalMetricsDTO } from "@/src/schemas/proposal.schema";
...
export function ProposalMetricsDrawer({
  ...
  onLoaded?: (metrics: ProposalMetricsDTO) => void;
}) {
  const [metrics, setMetrics] = useState<ProposalMetricsDTO | null>(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (!open) return;
    setLoading(true);
    getProposalMetrics(slug, proposalId)
      .then((data) => {
        if (data) {
          setMetrics(data);
          onLoaded?.(data);
                  ...
                  <ul className="space-y-1.5">
                    {metrics.views.map((view) => (
                      ...
                      >
                        ...
                        >
                          {view.reachedEnd ? "Concluiu" : `${view.scrolledPct}%`}
                        </span>
