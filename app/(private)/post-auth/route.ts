...
// Em route handler, `redirect()` não recebe basePath automático — usamos
// `withPublicUrl()` para garantir a URL absoluta correta no domínio público.
export async function GET(): Promise<never> {
  const session = await getAuthSession();

  if (!session.ok) {
    redirect(withPublicUrl("/sign-in").toString());
  }

  const path = await resolveWorkspacePath(session.value.user.id);
  redirect(withPublicUrl(path).toString());
}
