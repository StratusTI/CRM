...
import { CreateLineItemSchema } from "@/src/schemas/opportunity-line-item.schema";
...
export async function GET(_request: NextRequest, { params }: RouteContext) {
  ...
  const result = await OpportunityLineItemService.list(
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  const { slug, id } = await params;
  const result = await OpportunityLineItemService.create(
    session.value.user.id,
    slug,
    id,
    parsed.data,
