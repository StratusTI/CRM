...
export const ProfileService = {
  ...
  async list(userId: string, slug: string): Promise<Result<ProfileDTO[]>> {
    const ws = await resolveWorkspaceId(userId, slug, {
      resource: "members",
      action: "VIEW",
    });
    if (!ws.ok) return ws;
    const seeded = await ProfileRepository.ensureSystemProfiles(ws.value);
  ...
  async create(
    ...
    const taken = await ProfileRepository.existsByName(ws.value, input.name);
