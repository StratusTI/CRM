...
/**
 * Storage in-process para servir bytes temporariamente como URL pública. É o que
 * permite o upload de imagem no Instagram (o Graph API exige `image_url` HTTP,
 * não aceita upload direto): o service grava aqui, o Graph baixa, expira-se.
 *
 * Limitação: vive na memória do processo. Em ambiente multi-instância (várias
 * réplicas), a réplica que recebe o GET pode não ter o blob — para produção é
 * preciso trocar por S3/R2/Blob com TTL. Para single-process (dev/local) basta.
 */
...
/** Lê um blob (sem remover — o Graph pode tentar baixar mais de uma vez). */
