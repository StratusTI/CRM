...
import { socialNoPage } from "@/src/errors/app-error";
 ...
 * engajamento (`pages_read_engagement`), publicar (`pages_manage_posts`) e ler
 * métricas (`read_insights`). Postagens e insights são da **Página**, não do
 * perfil — por isso trocamos o token do usuário pelo Page token no fetchAccount.
 * Ao mudar esta lista, contas conectadas antes precisam reconectar.
 */
const SCOPES = [
  "public_profile",
  "pages_show_list",
...
export const facebookProvider: SocialProvider = {
  ...
  async fetchAccount(tokens): Promise<Result<SocialAccount>> {
    // Lista as Páginas administradas; cada uma traz seu próprio Page token. No
    // v1 conectamos a primeira Página (o nome fica visível na UI). Selecionar
    // entre várias é uma evolução futura.
    const params = new URLSearchParams({
    ...
    if (!page?.access_token) {
      // Sem Página concedida não há onde postar/ler insights — falha claramente
      // para o usuário reconectar e SELECIONAR uma Página (em vez de salvar uma
      // conexão quebrada com o token de usuário).
      return err(socialNoPage());
    }
