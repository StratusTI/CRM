import { TWITTER_CLIENT_ID, TWITTER_CLIENT_SECRET } from "@/lib/env/_server";
import { ok, type Result } from "@/src/lib/result";
import { expiresInToDate, getJson, postForm } from "./http";
import type { SocialAccount, SocialProvider, TokenSet } from "./types";

/**
 * Twitter/X OAuth 2.0 (Authorization Code com PKCE — exigido pelo X). Escopos:
 * ler/identificar o usuário (`users.read`, `tweet.read`), publicar (`tweet.write`),
 * anexar mídia (`media.write`) e renovar o token (`offline.access` → refresh
 * token). Cliente confidencial: a troca do code usa HTTP Basic com
 * `client_id:client_secret`. Ao mudar esta lista, contas já conectadas precisam
...
export const twitterProvider: SocialProvider = {
  ...
  buildAuthorizeUrl({ redirectUri, state, codeChallenge }) {
    const params = new URLSearchParams({
      ...
      scope: SCOPE,
      state,
      // PKCE é obrigatório no X; quando ausente o provedor recusa a autorização.
  ...
  async fetchAccount(tokens): Promise<Result<SocialAccount>> {
    ...
    return ok({
      ...
      // O @handle é mais útil que o nome de exibição para identificar a conta.
  ...
  async refreshAccessToken(refreshToken): Promise<Result<TokenSet>> {
    ...
    return ok({
      accessToken: result.value.access_token,
      // O X devolve um novo refresh token a cada renovação; o service preserva
      // o atual quando vier null.
      refreshToken: result.value.refresh_token ?? null,
      expiresAt: expiresInToDate(result.value.expires_in),
      scope: result.value.scope ?? SCOPE,
    });
