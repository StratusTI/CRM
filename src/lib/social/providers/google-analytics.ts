import { GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET } from "@/lib/env/_server";
import { ok, type Result } from "@/src/lib/result";
import { expiresInToDate, getJson, postForm } from "./http";
import type { SocialAccount, SocialProvider, TokenSet } from "./types";

/**
 * Google Analytics 4 via Google OAuth. Reusa as credenciais GOOGLE_* (mesmas do
 * YouTube/login), mas com escopo dedicado e fluxo próprio — não interfere no
 * better-auth nem na conexão do YouTube.
 *
 * Escopo: leitura de dados/admin (`analytics.readonly`). Pós-OAuth, escolhemos
 * automaticamente a 1ª propriedade GA4 via Admin API `accountSummaries` (mesma
 * estratégia da 1ª Page do Facebook) — o `externalId` armazenado é o
 * `properties/<id>` que o cliente da Data API exige.
 */
...
export const googleAnalyticsProvider: SocialProvider = {
  ...
  async fetchAccount(tokens): Promise<Result<SocialAccount>> {
    // Admin API account summaries — lista todas as contas + propriedades GA4
    // (web/app) que o usuário tem acesso, sem precisar de um property ID.
    ...
    // Pega a 1ª propriedade da 1ª conta que tenha alguma — espelha o pattern do
    // Facebook (1ª Page). Sem propriedades acessíveis, fica como "unknown" e o
    // service responde scope/conexão inutilizável.
