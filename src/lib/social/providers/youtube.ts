...
/**
 * YouTube via Google OAuth. Reusa as credenciais GOOGLE_* já usadas pelo login,
 * mas com fluxo próprio e escopo do YouTube — não interfere no better-auth.
 *
 * Escopos: ver canal/vídeos (`youtube.readonly`), métricas (`yt-analytics.readonly`)
 * e upload de vídeo (`youtube.upload`). Ao mudar esta lista, contas já conectadas
 * precisam reconectar para reconsentir — o service detecta scope ausente.
 */
const SCOPES = [
