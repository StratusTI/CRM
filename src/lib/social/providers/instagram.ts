...
import { socialIgNotLinked } from "@/src/errors/app-error";
 ...
 * trocamos o token de usuário por um Page token (Página → `instagram_business_account`)
 * e guardamos o id da conta do Instagram. Não usa `FACEBOOK_CONFIG_ID` do FB
 * Business Login porque ele já carrega escopos só de Página; aqui pedimos os
 * escopos IG explicitamente via `scope=`.
 */
...
export const instagramProvider: SocialProvider = {
  ...
  async exchangeCode({ code, redirectUri }): Promise<Result<TokenSet>> {
    ...
    // 2) token curto → token longo (~60 dias). Page tokens derivados de um token
    // longo não expiram, o que permite postar/ler insights sem refresh.
  ...
  async fetchAccount(tokens): Promise<Result<SocialAccount>> {
    // Para chegar ao IG: pegar as Páginas administradas, descobrir aquela que
    // tem uma conta IG Business/Creator vinculada e usar o **Page token** para
    // todas as chamadas do IG (Graph API delega o auth via Page). Sem IG
    // vinculado, falha claramente em vez de salvar uma conexão quebrada.
    const params = new URLSearchParams({
      fields:
    ...
    if (!pageWithIg) return err(socialIgNotLinked());

    const ig = pageWithIg.instagram_business_account;
    if (!ig) return err(socialIgNotLinked());
